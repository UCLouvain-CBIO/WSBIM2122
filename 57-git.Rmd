...
- Alice and Bob pull the latest changes and modify the empty `file1`
  by adding their name at the beginning of the file. Alice's file
  looks like this in her local repository:

```
Alice
```

  And Bob files looks like this in his local repository:
...
- Alice commits locally and pushes to the remote repository. `file1`
  in the remote and in Alice's local repo are now identical, and Git
  recorded that the change was adding a new line containing `Alice`.
- Bob now also tries to push but gets an error because he pushes a
...
```
  and then push again.
- Alice can not pull to retrieve Bob's changes from the remote.
...
username wisely as you might want to reuse it later and will have to
share it.
...
repository.  **We need to add 1 or 2 simple files to the repo**
...
- An expiration date, after that date the PAT won't work
  ...
  for your projects.
- What this PAT allows to do, it is recommended to select repo, user,
  gist and workflow.
...
thing needed from GitHub is the cloning https address of your repo. To
get it, go the repo page, click the big green button that says "<>
Code" and copy the HTTPS URL address.

To clone your GitHub repo on your computer, open RStudio and create a
new project. You'll then be able to chose to create a Version Control
project and choose Git. Then you just need to paste the HTTPS URL of
your GitHub repo and it will be cloned as an R project.
...
all that to GitHub. That's when you'll be asked to give your username
...
want to see all the changes that have been done to the repo.
...
- Avoid (Git) conflicts: using Git and Github is mostly a quite
  sailing on a calm sea. The only little annoying hickup are
  conflicts. It is best to avoid these by coordinate work.
- The best way to avoid conflicts is to always be in sync with the
  remote repository. To do so, always pull the latest changes before
  starting to work on a project.
- The best way to avoid conflicts is to help other always be in sync
  with the remote repository. To do so, regularly push your changes to
  the remote, so that others can pull them.
- To further avoid conflicts, try to modularise your project by
  splitting the work into multiple independement files. For an Rmd
  project:
...
pairs (called Alice and Bob below).

1. Alice creates a GitHub repository and adds `file1` to the repo, and
   adds Bob as a collaborator with write access: Settings >
   Collaborators (in the left panel) > Add people > Search and add a
   GitHub user.

2. Both Alice and Bob clone to remote repository and add their names
   to `file1`.
3. Alices commits her local changes and pushes them to the GitHub
   repository.
4. Bob commits his local changes and tries to push them to the GitHub
   repository.
5. Bob pulls the latest version from GitHub, and manually fixes
   conflict in `file1` by manually merging the conflicting lines and
   adding both names.
6. Bob commits his local changes and pushes to the remote repository.
...
- She could now send a pull request to merge changes from
  ...
  `UCLouvain-CBIO/WSBIM2122-GitHub-training`. The PR can be inspected
  and reviewed and merged by a member of the
  `UCLouvain-CBIO/WSBIM2122-GitHub-training`.
- Pull requests (of shortened PR) are also useful when multiple
