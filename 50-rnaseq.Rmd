...
This dataset corresponds to RNAseq data done on a a cell line
(cell lines "F") that was grown in acid condition or not.
The aim here is to test the influence of the acidity on the transcriptome.

```{r}
load("data/deseq2/counts_F_cells.rda")
load("data/deseq2/coldata_F_cells.rda")
coldata
head(counts)
dim(counts)
```
...
resultsNames() function shows the results available. Note that by default, R will choose 
a reference level for factors based on alphabetical order. Here acid condition is the reference level. 
This means that the fold changes evaluated for every gene will correspond to their expression level 
in cells grown in normal condition versus their expression level in cells grown at in acid condition.  
...
In this case, it seems more logical to use cells grown in normal condition as reference. 
Reference levels can be changed using the relevel( ) function. In this case you need to run DESeq()
function after the re-leveling operation.
...
[^log2C]:In this case, as we are testing the "condition_acid_vs_normal" coefficient, 
...
1. Which gene has the lowest padjusted value?
...
There is a peak at 0, but there is also a peak close to 0.8, and to a smaller extend 
a smaller peak at bout 1. 
...
by the independant filtering process. This means that DESeq2 does calculate 
a pvalue for every gene (except for those that have a basemean = 0), but 
the pvalue adjustement is computed later only on genes that pass the independant 
...
log2FC versus the mean of normalized counts. Genes with a padjusted value lower than 
...
In the MA-plot, we hope to observe some genes located in the the upper/lower right 
...
Recall that the most interesting features are those towards to top corners given 
that they have a small pvalues and large fold-changes.
The volcano-plot can be useful to define padjusted and log2FC thresholds. 
Based on this plot, we could for example decide (arbitrarly), to consider genes as 
...
as_tibble(counts(dds[best_genes$ENSEMBL, ], normalize = T), 
  ...
  ggplot(aes(x = sample, y = counts, fill = pH)) +
...
padjusted value (their padjusted value is between 0.05 and 1e-10).
...
1. Identify and inspect counts of the genes plotted in red in the following volcano-plot.
These genes have a very large log2FC (log2FC > 5) but are far from bearing the lowest padjusted value (their padjusted value is between 0.05 and 1e-10).

```{r}
selected_genes <- res_tbl %>% 
  filter(padj < 0.05 & padj > 1e-10 & log2FoldChange > 5)
...
The experiment was in reality done on 2 different cell lines (cell line "F" and cell line "H")
