...
$K_{ij}$ represents the raw count for gene i in sample j

$µ_{ij}$ represents the mean of gene i in sample j

$\alpha_i$ represents the dispersion of gene i
...
```{r, echo=FALSE, fig.align='center', out.width='60%', purl=TRUE}
knitr::include_graphics("./figs/VAR.png")
```
...
where $q_{ij}$ represents the normalised counts for gene i in sample j and $β_i$ represents 
the log2FC between conditions. $β_i$ coefficients are computed from the data and standard 
errors for the log2 fold changes are  calculated.
...
$$log2(q_{ij}) = \beta_0 + \beta_1\times x_j + \epsilon$$
...
groups, which is the same as saying that the log2FC = 0. A
