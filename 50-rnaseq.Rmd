...
ENSG00000198804 and ENSG00000248671 are two genes expressed respectively at a 
...
Generate a scatter plot representing the log counts of sample1 versus the 
log counts of sample2 (sample1 and sample2 are 2 control replicates).

Generate a scatter plot representing the "rlog transformed" counts of sample1 
versus the "rlog transformed" counts of sample2.
...
fig2 <- as_tibble(assay(rld[,1:2], normalize = TRUE)) %>% 
  ggplot(aes(x = sample1, y = sample2))+
  geom_point(alpha = 0.2) +
  geom_abline(slope = 1, color = "red") +
...
**Impact of number of replicate on the dispersion estimations.**

Create a a new fictive DESeqDataSet object containing only 2 replicates of each 
condition instead of the 3 replicates that we have in our `dds` object.

Inspect its dispersion plot. Compare with the dispersion plot of dds.
...
Using the `dispersions()` function, you can acceed to the dispersion values.
Compare on a scatterplot the dispersions values obtained with 2 replicates
...
on counts and on gene dispersion values[^FCShrinkage]. As seen in the following MA-plots, the 
shrinkage is much stronger for lowly than for highly expressed genes. 
