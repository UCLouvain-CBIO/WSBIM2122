...
```{r, message = FALSE}
go_ora <- enrichGO(gene = as.character(res_tbl$ENTREZID[res_tbl$padj < 0.05]),
...
```{r}
go_cc <- full_join(go_ora %>% filter(p.adjust < 0.01) %>% select(ID, p.adjust),
                   go_gsea %>% filter(p.adjust < 0.01) %>% select(ID, p.adjust),
                   by = c("ID" = "ID"))
...
The `enricher()` function can be used ...