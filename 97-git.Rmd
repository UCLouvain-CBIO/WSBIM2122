...
- Once a track change is accepted or rejected, it isn't possible to
  ...
  do allow this (and this requires on-line access).
...
the staging area (these can be done in one step), and commit the
changes.
...
you want to do collaborative work.  The RStudio interface then offers
a graphical user interface to git that can be easier than the
...
(classic)* and configure your token. You can configure :
...
Note: In some cases, users need to configure their email and name to
be able to commit using `git config user.email` and `user.name`.
