...
```{r cptac, echo = FALSE, fig.cap = "The CPTAC spike-in study design."}
...
```{r, fig.cap = "Three peptide level assays: raw data, log transformed and normalised.", fig.width = 15, fig.height = 5}
par(mfrow = c(1, 3))
...
```{r, fig.width = 12, fig.height = 6, fig.cap = "Peptide and protein level PCA analyses."}
pca_pep + pca_prot
```
...
```{r, warning = FALSE, fig.width = 12, fig.height = 6, fig.cap = "Peptide and protein expression profile."}
longFormat(cptac["P02787ups|TRFE_HUMAN_UPS", ,
    ...
    geom_point(size = 3) +
    geom_line(aes(condition = rowname)) + 
    facet_grid(~ assay) +
...
other `r sum(res$TP) - 1` expected spike-in proteins. As the figure
below shows, it is possible to substantially improve these results.
...
(namely C) and a two lab, tallying now 27 samples.
...
```
The full design is shown below and is available [here](./datac/ptac_coldata.csv)
