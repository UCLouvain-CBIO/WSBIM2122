...
1. Values that are missing due to the absence (or extremely low
   ...
   and their pattern **aren't random**. A protein missing due to
...
When downstream analyses permit, it might thus be safer not to impute data and
deal explicitly with missing values. This is possible for hypothesis testing
and for dimensionality reduction using methods specifically designed to handle
missing values, such as NIPALS [@wold_estimation_1966] or BPCA
[@bishop_bayesian_1998].
...
enables the consistent application of linear models to normally
...
borrows information across features to estimate the standard
error[^shrinkage] and calculate (so called moderate) t
statistics. This approach is demonstrably more powerful than a
standard t-tests when the number of samples is lot.

[^shrinkage]: alike the shinkage gene-wise dispersion estimates see in
    `DESeq2`.

Note that other methods directly developed for proteomics are also available,
such as msqrob2 [@goeminne_peptide-level_2016; @goeminne_msqrob_2020] or limpa
[@li_quantification_2025]
