...
$$expression = \beta_0 + \beta_1 \times x + \epsilon$$
...
$$y = \beta_0 + \beta_1 \times group + \beta_i \times ID + \epsilon$$
...
This is partly due to the cost of using a more complex model and
reducing the degrees of freedom that affect the calculation of the
statistics, and that the question asked is slighly different. The
coefficient `cell_typeB` in model without interaction tests for
differences in cell types by taking into account that there are two
types. In the model with interaction, the test focuses in the cell
type effect in the reference condition only.
...
same model is applied to all genes and the significance of
...
types), then the more complex model (with an interaction) it
