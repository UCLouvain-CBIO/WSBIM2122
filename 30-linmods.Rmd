...
- If we return to the previous example for gene 3, we see that in both
  ...
  albeit with a much smaller p-value in the simpler model. This is due
  to the cost of using a more complex model and reducing the degrees
  of freedom that affect the calculation of the statistics.
...
- If we look at gene 5, we see that we miss out several significant
  effects if we use the simpler model, without interaction.
...
experiment[^hypo]. It there are reasons to believe that and
interaction effect is relevant, then the more complex model it
appropriate. If no such interaction is expected, then testing for it
penalises the power of the tests.

[^hypo]: before even generating any data.
