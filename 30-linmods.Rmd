...
- If we look at gene 5, we see that we miss out several significant
  effects if we use the simpler model, without interactions.
...
genes, we can't explore each gene separately with a different
model. The same model is applied to all genes and the significance of
coefficient(s) of interest are considered.

Solving this dilema isn't a statistical issue, but a scientific question.
