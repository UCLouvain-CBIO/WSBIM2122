...
For simplicity, we are going to ignore the location of the peptide
modifications by removing the `modLocation` column.
...
Choose an MS2 spectrum that has an `MS.GF.RawScore` greater than 100
and visualise it with its fragment ions using the
`plotSpectraWithFragments()` function, passing the `Spectra` object
containing the selected spectrum and its sequence.
...
```{r, eval = FALSE}
which(ms2$MS.GF.RawScore > 100)
i <- 4325
plotSpectraWithFragments(ms2[i],
                         ms2$sequence[i],
                         labelPos = 3)
```
