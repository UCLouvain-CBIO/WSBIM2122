...
BiocManager::install("lgatto/joinSpectraData")
```

The `joinSpectraData` is also (temporarily) needed.
...
- For how long was acquition time? Use the `rtime()` accessor to 
...
[^noteID]: A more rigorous processing of these identifications would
	...
	`r BiocStyle::Biocpkg("MSnID")` is a highly recommended.
...
identification data (the `id` variable), it is possible to
join[^joinSpectraData] the latter to the former's spectra data:


[^joinSpectraData]: The `joinSpectraData()` function is available in
    the `joinSpectraData` package, that can be installed with
    `BiocManager::install("lgatto/joinSpectraData")`. This function
    will be included in the `Spectra` package.
...
```{r, fig.height = 9, fig.width = 6, message = FALSE}
...
These different quantitation techniques come with their respective and
distinct challenges, such as large quantities of raw data processing,
data transformation and normalisation, and different underlying
statistical models for the quantitative data (count data for spectral
counting, continuous data for the others).
